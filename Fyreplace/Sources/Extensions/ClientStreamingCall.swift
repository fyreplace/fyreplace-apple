...
extension ClientStreamingCall where RequestPayload == FPBImageChunk, ResponsePayload == Google_Protobuf_Empty {
    func upload(image: Data?) {
        ...
        for i in 0..<chunkCount {
            ...
            _ = sendMessage(FPBImageChunk.with { $0.data = image.subdata(in: start..<end) })
        }
